...
public class DetailPaneel extends GridPane {
	...
	private void buildGui() {
		...
		Button btnStorten = new Button("storten");
		Button btnAfhalen = new Button("afhalen");
		this.add(btnStorten, 0, 5);
		this.add(btnAfhalen, 2, 5);
		GridPane.setHalignment(btnAfhalen, HPos.RIGHT);

		btnStorten.setOnAction(new javafx.event.EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent arg0) {
				verwerkBedrag(TransactieType.STORTEN);
			}
		});
		btnAfhalen.setOnAction(new javafx.event.EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				verwerkBedrag(TransactieType.AFHALEN);
			}
		});
	...
	private void verwerkBedrag(TransactieType transactieType) {
		try {
			...
			System.err.println("Kon geen double parsen in het meegegeven bedrag.");
