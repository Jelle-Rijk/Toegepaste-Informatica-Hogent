...
export default class TriviaComponent {
  ...
  async #getData() {
    await fetch(this.#url)
      .then((res) => res.json())
      .then((json) => this.#triviaRepository.addTrivias(json.results))
      .catch((err) => {
        document.getElementById("answer").innerText = err;
      });
  }
  ...
  #showTrivia() {
    ...
    questions.insertAdjacentHTML(
      "afterbegin",
      `Question: ${this.#triviaRepository.numberOfAnswers + 1} / ${this.#triviaRepository.numberOfTrivias}`,
    );
  ...
  #sendClickHandler(trivia) {
    ...
    const answer = document.querySelector('input[name="group"]:checked').value;
    const correct = this.#triviaRepository.checkAnswer(answer);

    // TODO
    // aantal correcte antwoorden op het totaal aantal antwoorden
    document.getElementById("correct").innerText =
      `Correct answers: ${this.#triviaRepository.correctAnswers} / ${this.#triviaRepository.numberOfAnswers}`;

    // TODO
    // tonen van het correcte antwoord
    const correctAnswer = document.getElementById("answer");
    correctAnswer.insertAdjacentHTML(
      "afterbegin",
      `<p>The correct answer was: <span>${trivia.correctAnswer}</span></p>`,
    );
