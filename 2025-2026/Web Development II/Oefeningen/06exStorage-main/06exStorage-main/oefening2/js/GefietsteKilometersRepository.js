...
export default class GefietsteKilometersRepository {
  ...
  geefJaren() {
    return [...new Set(this.#repository.map((obj) => obj.jaar))];
  }

  geefGefietsteKilometersVoorEenJaar(jaar) {
    return this.#repository.find((el) => el.jaar === jaar)?.aantalKilometers;
  }

  wijzigGefietsteKilometers(jaar, waarden) {
    const kmJaar = this.#repository.find((el) => el.jaar === jaar);
    kmJaar.aantalKilometers = waarden;
  }
