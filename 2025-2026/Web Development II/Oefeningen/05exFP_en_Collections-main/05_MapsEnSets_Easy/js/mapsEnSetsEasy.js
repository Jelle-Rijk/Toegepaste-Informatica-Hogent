...
word = prompt("Geef een nederlands woord");
while (word) {
  if (en_fr_dict.has(nl_en_dict.get(word)))
...
for (const nlWord of nl_en_dict.keys()) console.log(nlWord);
...
for (const [engels, frans] of en_fr_dict.entries())
  console.log(`${engels}: ${frans}`);
...
const telZelfdeWoorden = (woordenboek) => {
  return woordenboek
    .entries()
    .reduce((result, entry) => (result += entry[0] === entry[1] ? 1 : 0), 0);
};
// Gebruik de functie en log naar de console voor onze beide dictionaries
// hoeveel identieke woorden ze bevatten
console.log(telZelfdeWoorden(nl_en_dict));
console.log(telZelfdeWoorden(en_fr_dict));
...
const krishnaSet = new Set();
krishna.forEach((el) => krishnaSet.add(el));

for (const el of krishnaSet) console.log(el);

// Maak nu gebruik van de spread syntax om met een one-liner
// een *array* van unieke waarden te krijgen uit de array krishna
const krishnaUniek = [...new Set(krishna)];
console.log(krishnaUniek);

// Maak een array met 3 *verschillende* random getallen tussen 1 en 6
const randomGetallenSet = new Set();
while (randomGetallenSet.size < 3) {
  randomGetallenSet.add(Math.floor(Math.random() * 6) + 1);
}
const randomGetallenArray = [...randomGetallenSet];
console.log(randomGetallenArray);
