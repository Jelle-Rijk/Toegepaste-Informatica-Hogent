...
SELECT * FROM titles 
WHERE primaryTitle LIKE '%Friends%'
...
-- 6. Geef alle episodes uit seizoen 2 van Friends
SELECT *
FROM titles t JOIN episodes e ON t.tconst = e.tconst 
WHERE t.titleType = 'tvEpisode' AND  e.seasonNumber = 2
AND e.parentTconst = (SELECT tconst FROM titles WHERE primaryTitle = 'Friends' AND titleType = 'tvSeries');